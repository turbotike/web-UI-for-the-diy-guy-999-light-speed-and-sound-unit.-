...
uint16_t RZ7886_FREQUENCY = 600;     // 500 Hz is recommended. It is not audible, if virtual engine sound is running. Higher frequencies may overheat the driver IC!
...
const float CUTOFF_VOLTAGE = 3;        // Usually 3.3 V per LiPo cell. NEVER below 3.2 V!
const float FULLY_CHARGED_VOLTAGE = 4; // Usually 4.2 V per LiPo cell, NEVER above!
const float RECOVERY_HYSTERESIS = 0;   // around 0.2 V
...
uint32_t RESISTOR_TO_BATTTERY_PLUS = 9000; // Value in Ohms (Ω), for example 10000
